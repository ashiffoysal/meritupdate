...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <div class="toolbar" id="kt_toolbar">
        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            ...
            <div data-kt-place="true" data-kt-place-mode="prepend" data-kt-place-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center me-3 flex-wrap mb-5 mb-lg-0 lh-1">
                <!--begin::Title-->
                <h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3">Video Watch</h1>
                <!--end::Title-->
            </div>
                    ...
                    <div class="card mb-5 mb-xl-8">
                        ...
                        <div class="card-header border-0 pt-5">
                            <h3 class="card-title align-items-start flex-column">
                                <span class="card-label fw-bolder fs-3 mb-1">All Watching Records</span>
                            ...
                            <div class="table-responsive">
                                ...
                                <table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3" id="dataTableExample1" class="data-table" style="width:100%">
                                    ...
                                    <thead class="text-center">
                                        ...
                                        <th class="min-w-140px">Video Title</th>
                                        <th class="min-w-140px">Watch Record</th>
                                     
                                    </thead>
                                    <tbody class="text-center">
                                    @foreach($alldata as $key => $data)
                                     @php
                                            $ifhasSubject=DB::table('branch_tutor_subjects')->where('subject_id',$data->sub_cate)->where('tutor_id',$data->id)->first();
                                        @endphp
                                        ...
                                        <tr>
                                           ...
                                           <td>
                                          @php
                                                $watchper=DB::table('video_watch')->where('video_id',$data->id)->where('user_id',$id)->first();
                                            @endphp
                                            <td>
                                                @if($watchper)
                                                <div class="progress" role="progressbar" aria-label="Example with label" aria-valuenow="{{ $watchper->watchpercentage }}" aria-valuemin="0" aria-valuemax="100">
  ...
  <div class="progress-bar" style="width: 25%">0%</div>
                                    ...
                                    @foreach($allIndividualdata as $key => $data)
                                    
                                    @php
                                        $video=DB::table('video_tutorial')->where('id',$data->video_id)->first();
                                    @endphp
                                     <tr>
                                            <td class="text-dark fw-bolder text-hover-primary fs-6"> {{ ++$key }} </td>
                                            <td>
                                                <a href="#" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ $video->title ?? '' }}</a>
                                            </td>
                                           <td>
                                          
                                            <td>
                                             
                                                <div class="progress" role="progressbar" aria-label="Example with label" aria-valuenow="{{ $data->watchpercentage }}" aria-valuemin="0" aria-valuemax="100">
  <div class="progress-bar" style="width: {{ $data->watchpercentage }}%">{{ round($data->watchpercentage) }} %</div>
